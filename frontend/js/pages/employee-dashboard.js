...
async function loadDashboardData(user) {
  ...
  try {
    ...
    const todaySchedule = schedules.find(s => {
      const d = new Date(s.day_of_week !== undefined ? s.day_of_week : s.date);
      return d.getDay() === todayDay || s.day_name === dayNames[todayDay];
    });
    ...
    container.innerHTML =
      ...
      '<div style="display:flex;gap:16px;margin-bottom:24px;flex-wrap:wrap;">' +
        '<button id="clock-in-btn" style="flex:1;min-width:200px;padding:20px;background:#166534;border:2px solid #22c55e;border-radius:12px;color:#fff;font-size:1.2rem;font-weight:700;cursor:pointer;transition:all 0.2s;">' +
          '<div style="font-size:2rem;margin-bottom:8px;">&#9201;</div>Clock In' +
        '</button>' +
        '<button id="clock-out-btn" style="flex:1;min-width:200px;padding:20px;background:#7f1d1d;border:2px solid #ef4444;border-radius:12px;color:#fff;font-size:1.2rem;font-weight:700;cursor:pointer;transition:all 0.2s;">' +
          '<div style="font-size:2rem;margin-bottom:8px;">&#9202;</div>Clock Out' +
        '</button>' +
      '</div>' +
            ...
            '<div style="display:flex;flex-direction:column;gap:8px;">' +
              recentRecords.map(function(r) {
                ...
                return '<div style="background:#1a1f2e;border:1px solid #2a3040;border-radius:8px;padding:10px 14px;display:flex;justify-content:space-between;align-items:center;">' +
                  '<span style="color:#ccc;font-size:0.85rem;">' + (r.date || '') + '</span>' +
...
function showPinModal(action, user, isClockIn) {
  var pin = '';
  var title = action;
  var subtitle = 'Enter your PIN to ' + action.toLowerCase();

  var html =
    '<div style="text-align:center;padding:10px 0;">' +
      '<div style="color:#aaa;margin-bottom:20px;font-size:0.95rem;">' + subtitle + '</div>' +
      ...
      '</div>' +
      '<div style="display:grid;grid-template-columns:repeat(3,70px);gap:8px;justify-content:center;margin:0 auto 20px;">' +
        [1,2,3,4,5,6,7,8,9,'',0,'&#9003;'].map(function(key) {
          if (key === '') return '<div></div>';
          var display = key === '&#9003;' ? '&#9003;' : key;
          var cls = key === '&#9003;' ? 'pin-key pin-key-del' : 'pin-key';
          return '<button class="' + cls + '" data-key="' + key + '" style="width:70px;height:56px;border-radius:10px;border:1px solid #30363d;background:#161b22;color:#e2e8f0;font-size:1.2rem;font-weight:600;cursor:pointer;transition:all 0.15s;display:flex;align-items:center;justify-content:center;">' + display + '</button>';
        }).join('') +
      '</div>' +
      '<button id="pin-clear-btn" style="background:none;border:1px solid #30363d;color:#888;padding:6px 18px;border-radius:6px;cursor:pointer;font-size:0.85rem;margin-bottom:8px;">Clear</button>' +
  ...
  setTimeout(function() {
    ...
    function updateDisplay() {
      if (dotsEl) dotsEl.textContent = pin.split('').map(function() { return '\u2022'; }).join('');
    }

    document.querySelectorAll('.pin-key').forEach(function(btn) {
      btn.addEventListener('click', function() {
        var key = btn.getAttribute('data-key');
        if (key === '\u232b' || key === '9003' || btn.classList.contains('pin-key-del')) {
          pin = pin.slice(0, -1);
        } else {
  ...
  submitBtn.id = 'pin-submit-btn';
  submitBtn.textContent = action;
  submitBtn.style.cssText = 'width:100%;padding:12px;border:none;border-radius:8px;background:' + (isClockIn ? '#22c55e' : '#ef4444') + ';color:#fff;font-size:1rem;font-weight:600;cursor:pointer;margin-top:12px;';

  setTimeout(function() {
    var modalBody = document.querySelector('.modal-body') || document.querySelector('#modal-body');
    if (modalBody) {
      var modalContent = modalBody.closest('.modal') || modalBody.parentElement;
      if (modalContent) {
        var existingSubmit = document.getElementById('pin-submit-btn');
        if (!existingSubmit) modalContent.appendChild(submitBtn);
      }
    }
